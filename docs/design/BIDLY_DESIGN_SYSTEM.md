...
`price-positive` means a verified favourable comparison and is never the sole indicator. `auction-active` is informational, not a winning state.

## Component layers

1. Primitives: button, surface, input/field, badge, focus/skip behaviour.
2. Brand and icons: Bidly logo/mark plus one internal outline icon set.
